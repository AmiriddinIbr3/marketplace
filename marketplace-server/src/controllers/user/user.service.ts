...
import { PhoneService } from 'grpc/services/clients/phone/phone.service';
...
export class UserService {
  ...
  async create(
    ...
    const { countryCode, valid } = await this.phoneService.checkPhone(phone);
    if (!valid) {
  ...
  async getNotices(userId: string) {
    const notices = await this.noticeDatabase.findNoticesByUserId(userId);
